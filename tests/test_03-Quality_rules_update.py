...
# Test name: t03 - Quality rules update
import pytest
import time
import json
...
class Test_03_Quality_rules_update:
  ...
  def test_03_Quality_rules_update(self):
    ...
    self.driver.execute_script("//SETDESC:Data Quality Rules changes identified.")
    ...
    assert len(self.driver.find_elements(By.CSS_SELECTOR, "input[name=\"update[dataquality]\"]")) > 0
    None if (element := self.driver.find_element(By.CSS_SELECTOR, "input[name=\"update[dataquality]\"]")).is_selected() else element.click()
