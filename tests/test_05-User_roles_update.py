...
# Test name: t05 - User roles update
import pytest
import time
import json
...
class Test_05_User_roles_update:
  ...
  def test_05_User_roles_update(self):
    ...
    self.driver.execute_script("//SETDESC:User Roles changes identified.")
    ...
    assert len(self.driver.find_elements(By.CSS_SELECTOR, "input[name=\"update[roles]\"]")) > 0
    None if (element := self.driver.find_element(By.CSS_SELECTOR, "input[name=\"update[roles]\"]")).is_selected() else element.click()
